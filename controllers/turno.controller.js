      ...
      Jornadas.findById(_cliente.jornada, function(err, jor) {
        if (_cliente.confirmado == "") {
          try {
            ...
            console.error(
              "Error al confirmar " + id_final + " Turno con error " + err
            );
...
exports.confirmarTurno = function(req, res) {
  ...
  Turno.findById(myquery, function(err, _cliente) {
    if (_cliente != undefined) {
      console.log("reconozco cliente " + _cliente.nombre);
      Jornadas.findById(_cliente.jornada, function(err, jor) {
        Veterinarios.findOne({ $or: [{ nombre: jor.veterinario }] }, function(
          err,
          veterinario
        ) {
...
exports.cancelar_turno = function(req, res) {
  ...
  Turno.findOneAndUpdate(myquery, newvalues, function(err, data) {
    // modificado para cancelar turno con motivo 02/02/2020
...
async function altaDeTurno(req, res) {
  try {
    let cantidadDeTurnos = await cuentoTurnos(
      req.body.jornada,
      req.body.telefono,
      req,
      res
          ...
          if (jor.activa == false) {
            ...
            if (data.confirmado == "0") {
              ...
              respuesta = [
                data.id,
                data.nombre,
                moment(data.hora).format("DD/MM/YYYY, HH:mm"),
                jor.localidad,
                jor.direparcial
              ];
