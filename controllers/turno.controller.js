      ...
      try {
          ...
          if (_contador >= _grupo) {
            ...
            console.log("Mandando mail " + turno.mail + ' - ' + turno.nombre + ' - ' + _localidad, _precio + ' - ' + moment(_fecha).format('D/M/YYYY') + ' - ' + moment(hora_nueva).tz("America/Argentina/Buenos_Aires").format('HH:mm') + ' - ' + _direparcial, turno.id);
             ...
             mail_confirmacion(turno.mail, turno.nombre, _localidad, _precio, moment(_fecha).format('D/M/YYYY'), moment(hora_nueva).tz("America/Argentina/Buenos_Aires").format('HH:mm'), _direparcial, turno.id);
        }
        catch(error) {
          console.error(error);
...
function sumar_mediahora(hora){
    ...
    var hora_final = moment(hora).add(30, 'minutes').tz("America/Argentina/Buenos_Aires").format('HH:mm');
