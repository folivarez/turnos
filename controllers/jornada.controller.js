...
exports.jornada_create = function(req, res) {
  ...
  Jornada.save()
    .then(item => {
      ...
      console.log(
        "fecha " +
          moment(req.body.fecha).format("dddd, MMMM Do YYYY, h:mm:ss a")
      );
...
exports.jornadas_publicas = function(req, res) {
    var myquery = { activa: true };
  
    Jornadas.find(myquery, 'localidad hora_prox_turno', function(err, data) {
      if (err) return next(err);
