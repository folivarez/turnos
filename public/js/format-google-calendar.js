...
window.formatGoogleCalendar = function () {
    ...
    var renderList = function renderList(data, settings) {
        ...
        for (i in upcomingResult) {
            ...
            //console.log('evento ' + transformationList(upcomingResult[i], settings.itemsTagName, settings.format));
    ...
    var transformationList = function transformationList(result, tagName, format) {
        ...
        var dateFormatted = getFormattedDate(dateStart, dateEnd, dayNames, moreDaysEvent, isAllDayEvent),
            ...
            i;
             ...
             //console.log(dateStart + ' - ' +  dateEnd+ ' - ' +  dayNames+ ' - ' +  moreDaysEvent+ ' - ' +  isAllDayEvent);
        ...
        for (i = 0; i < format.length; i++) {
            ...
            if (format[i] === '*summary*') {
                output = output.concat('<h5 class="center centerh5">' + summary + '</h5>');
            } else if (format[i] === '*date*') {
                //console.log('date - ' +  dateFormatted);
                
                if(unavez){
                    mostrarDia(dateFormatted.substring(8,10).trim());
                    unavez = false;
                }
                ...
                output = output.concat('<p class="center fecha">' + dateFormatted + '</p>');
    ...
    var getMonthName = function getMonthName(month) {
        var monthNames = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Sep', 'Oct', 'Nov', 'Dic'];
    ...
    var formatDateSameDay = function formatDateSameDay(dateStart, dateEnd, dayNames, moreDaysEvent, isAllDayEvent) {
        ...
        return dayNameStart + ' ' + dateStart[0] + ' de ' + getMonthName(dateStart[1])  ;
    };
        ...
        if (dateStart[0] === dateEnd[0]) {
            if (dateStart[1] === dateEnd[1]) {
                if (dateStart[2] === dateEnd[2]) {
                    ...
                    formattedDate = formatDateSameDay(dateStart, dateEnd, dayNames, moreDaysEvent, isAllDayEvent);
                    //console.log('toy aca - ' + formattedDate);
                } else {
                    //month day, year - month day, year
